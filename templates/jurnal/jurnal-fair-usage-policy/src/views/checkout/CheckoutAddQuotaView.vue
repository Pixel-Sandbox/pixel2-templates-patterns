  ...
  <mp-box position="relative">
    ...
    <mp-flex as="main" max-height="calc(100vh - 60px)">
                    ...
                    <mp-table :is-hoverable="false">
                      <mp-table-head bg="gray.50">
                        <mp-table-row>
                          ...
                          <mp-table-cell as="th" scope="col" width="48px">
                          </mp-table-cell>
                        </mp-table-row>
                      </mp-table-head>
                      <mp-table-body>
                        <mp-table-row v-for="quota in quotas" :key="quota">
                          <mp-table-cell as="td" scope="row">
                            <mp-text
                              :is-link="quota === 'Faktur penjualan'"
                              line-height="md"
                              @click.native="isCheckoutModalAddQuotaOpen = true"
                            >
                              {{ quota }}
                            </mp-text>
                            <mp-text
                              ...
                              >
                                <mp-popover-list>
                                  <mp-popover-list-item
                                    @click="handleAdd('Faktur penjualan')"
                                    v-if="!quotas.includes('Faktur penjualan')"
                                    >Faktur penjualan</mp-popover-list-item
                                  >
                                  <mp-popover-list-item
                                    @click="handleAdd('Gudang')"
                                    v-if="!quotas.includes('Gudang')"
                                    >Gudang</mp-popover-list-item
                                  >
                                  <mp-popover-list-item
                                    @click="handleAdd('Pengguna tambahan')"
                                    v-if="!quotas.includes('Pengguna tambahan')"
              ...
              <mp-box border-width="1px" p="6" bg="white" rounded="sm">
                ...
                <mp-box mt="5">
                  ...
                  <mp-flex justify="end" mt="5">
                    <mp-button-group>
                      <mp-button variant="ghost" @click="isPromptOpen = true"
                        >Kembali</mp-button
                      >
      ...
      <CheckoutModalAddQuota
        :is-open="isCheckoutModalAddQuotaOpen"
        @handleClose="handleCloseModalAddQuota"
        @handleAdd="handleAddModalAddQuota"
      />
      <ModalPrompt
...
export default {
  ...
  methods: {
    handleAdd(name) {
      ...
      if (this.$refs["tambah-lainnya"]?._isOpen)
        this.$refs["tambah-lainnya"]?.closePopover();
    },
    handleDelete(name) {
